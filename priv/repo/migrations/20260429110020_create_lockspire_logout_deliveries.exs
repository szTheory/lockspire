defmodule Lockspire.TestRepo.Migrations.CreateLockspireLogoutDeliveries do
  ...
  def change do
    create lockspire_table(:lockspire_logout_deliveries) do
      add(:delivery_id, :text, null: false)

      add(
        :logout_event_id,
        lockspire_references(:lockspire_logout_events, on_delete: :delete_all),
        null: false
      )
    ...
    create(lockspire_unique_index(:lockspire_logout_deliveries, [:delivery_id]))

    create(
      lockspire_unique_index(:lockspire_logout_deliveries, [
        :logout_event_id,
        :client_id,
        :channel
      ])
    ...
    create(lockspire_index(:lockspire_logout_deliveries, [:logout_event_id]))
    create(lockspire_index(:lockspire_logout_deliveries, [:client_id]))
    create(lockspire_index(:lockspire_logout_deliveries, [:status]))
  end
end
