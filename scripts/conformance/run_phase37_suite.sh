...
if [[ "${MODE}" == "hosted" ]]; then
  PROVIDER_DISCOVERY_URL="${LOCKSPIRE_PHASE37_PROVIDER_DISCOVERY_URL:-}"
  PROVIDER_BASE_URL="${LOCKSPIRE_PHASE37_PROVIDER_BASE_URL:-}"

  if [[ -z "${PROVIDER_DISCOVERY_URL}" ]]; then
    echo "LOCKSPIRE_PHASE37_PROVIDER_DISCOVERY_URL is required when LOCKSPIRE_PHASE37_MODE=hosted" >&2
    exit 1
  fi
        ...
        "tasks": [
            {
                ...
                "commands": [
                    ...
                    ["text", "name", "password", "phase37-password", "optional"],
...
config = {
    ...
    "client": {"client_name": "lockspire-phase37-local-client"},
    "browser": browser,
    "override": {
...
find "${ARTIFACT_DIR}" -type f | sort >"${ARTIFACT_DIR}/artifact-files.txt"
echo "Phase 37 conformance artifacts saved to ${ARTIFACT_DIR}"
