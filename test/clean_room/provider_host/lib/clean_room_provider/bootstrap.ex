defmodule CleanRoomProvider.Bootstrap do
  ...
  def provision!(handoff_dir) do
    ...
    {:ok, bearer} = Lockspire.Clients.register_client(client_attrs("clean-room-bearer", @bearer_redirect))
    {:ok, dpop} = Lockspire.Clients.register_client(client_attrs("clean-room-dpop", @dpop_redirect))
    ...
    %{bearer_client_id: bearer.client.client_id, dpop_client_id: dpop.client.client_id, bearer_policy: bearer_policy}
  end

  defp client_attrs(client_id, redirect_uri) do
    %{client_id: client_id, client_type: :confidential, name: client_id, redirect_uris: [redirect_uri], allowed_scopes: ["openid", "profile", "read:billing"], allowed_grant_types: ["authorization_code", "refresh_token"], allowed_response_types: ["code"], token_endpoint_auth_method: :client_secret_basic, metadata: %{resource_indicators: ["billing-api"]}}
  end
