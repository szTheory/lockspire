defmodule Lockspire.ClientMetadata do
  ...
  def build_dcr_client(metadata, resolved, iat_record, credentials)
    ...
    auth_method = metadata |> Map.get("token_endpoint_auth_method", "client_secret_basic") |> atomize_auth_method()
    ...
    %Client{
      ...
      token_endpoint_auth_signing_alg: atomize_token_endpoint_auth_signing_alg(Map.get(metadata, "token_endpoint_auth_signing_alg")),
      pkce_required: true,
      subject_type: :public,
      ...
      active: true,
      dpop_policy: if(Map.get(metadata, "dpop_bound_access_tokens", false), do: :dpop, else: :bearer),
      ...
      authorization_signed_response_alg: atomize_alg(Map.get(metadata, "authorization_signed_response_alg")),
      authorization_encrypted_response_alg: atomize_authorization_encryption_alg(Map.get(metadata, "authorization_encrypted_response_alg")),
      authorization_encrypted_response_enc: atomize_authorization_encryption_enc(Map.get(metadata, "authorization_encrypted_response_enc")),
      security_profile: atomize_security_profile(Map.get(metadata, "security_profile", "inherit")),
      client_id_issued_at: now,
      client_secret_expires_at: DateTime.add(now, Map.get(resolved, :default_client_secret_lifetime_seconds, 0) || 0, :second),
      metadata: metadata |> Map.take(["client_uri"]) |> Map.reject(fn {_key, value} -> is_nil(value) end)
    }
  end
  ...
  def check_fapi_signing_readiness(_old, :fapi_2_0_message_signing), do: {:error, :missing_compliant_active_key}
  def check_fapi_signing_readiness(_old, :fapi_2_0_security), do: {:error, :missing_compliant_publishable_key}
  ...
  defp validate_logout_propagation(attrs, redirect_uris) do
    ...
    |> maybe_session_required_error(attrs, :backchannel_logout_uri, :backchannel_logout_session_required)
    |> maybe_session_required_error(attrs, :frontchannel_logout_uri, :frontchannel_logout_session_required)
  ...
  defp maybe_logout_uri_error(errors, attrs, field) do
    case fetch_attr(attrs, field) do
      nil -> errors
      uri -> case Shape.valid_redirect_uri?(uri) do
        :ok -> errors
  ...
  defp maybe_session_required_error(errors, attrs, uri_field, session_field) do
    if normalize_boolean(fetch_attr(attrs, session_field)) and is_nil(normalize_string(fetch_attr(attrs, uri_field))) do
  ...
  defp maybe_frontchannel_origin_error(errors, attrs, redirect_uris) do
    case normalize_string(fetch_attr(attrs, :frontchannel_logout_uri)) do
      nil -> errors
      uri -> if origin_matches?(uri, redirect_uris), do: errors, else: [issue(:frontchannel_logout_uri, :frontchannel_logout_origin_mismatch, uri) | errors]
    end
  end

  defp origin_matches?(uri, redirect_uris), do: Enum.any?(redirect_uris, &(uri_origin(&1) == uri_origin(uri) and uri_origin(uri) != :invalid))
  defp uri_origin(uri) do
    case URI.parse(uri) do
      %URI{scheme: scheme, host: host, port: port} when scheme in ["http", "https"] and is_binary(host) and host != "" -> {scheme, host, port || if(scheme == "https", do: 443, else: 80)}
      _ -> :invalid
    end
  end
  ...
  defp parse_scope(scope) when is_binary(scope), do: scope |> String.split(" ", trim: true) |> Enum.uniq()
