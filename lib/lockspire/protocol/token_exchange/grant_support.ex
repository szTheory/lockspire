defmodule Lockspire.Protocol.TokenExchange.GrantSupport do
  ...
  def handle_code_exchange(client, code, code_hash, params, context, request) do
    with_dependencies(request, fn dependencies ->
      Internal.handle_code_exchange(
        client,
        code,
        code_hash,
        params,
  ...
  def authenticate_client(params, authorization, request) do
    with_dependencies(request, fn dependencies ->
      Internal.authenticate_client(params, authorization, request, dependencies)
    end)
  end

  def fetch_authorization_code(params, request) do
    with_dependencies(request, fn dependencies ->
      Internal.fetch_authorization_code(params, request, dependencies)
    end)
  end

  def fetch_device_authorization_for_exchange(params, %Client{} = client, request) do
    with_dependencies(request, fn dependencies ->
      Internal.fetch_device_authorization_for_exchange(params, client, request, dependencies)
    end)
  end

  def fetch_ciba_authorization_for_exchange(params, %Client{} = client, request) do
    with_dependencies(request, fn dependencies ->
      Internal.fetch_ciba_authorization_for_exchange(params, client, request, dependencies)
    end)
  end
  ...
  def emit_failure(%Error{} = error, params, request),
    do:
      with_dependencies(request, fn dependencies ->
        Internal.emit_failure(Compatibility.to_neutral(error), params, request, dependencies)
      end)

  defp with_dependencies(request, fun) when is_function(fun, 1) do
    case LegacyOptions.from_request(request) do
      {:ok, dependencies} -> fun.(dependencies) |> to_public_result()
