...
Before merging a Release Please PR for the root package, confirm this checked-in release-candidate contract end to end. The auto-merge workflow uses successful canonical CI for the exact candidate commit and revalidates the post-merge commit as current `main`; the Release Please PR is generated by `GITHUB_TOKEN` and contains only release metadata files.

1. Confirm canonical CI is green for the exact candidate commit; if merging manually because the guard does not apply, run `mix ci` or review equivalent SHA-matched CI evidence, then require the merge commit's own successful CI before publication.
