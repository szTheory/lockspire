...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 131-01-T1 | 131-01 | 1 | INST-01 | T-131-01, T-131-02 | Guarded admin routes precede public forwarding; the actual rendered macro is compiled and inspected through `Phoenix.Router.routes/1`. | compile + integration | `mix test test/integration/install_generator_test.exs` | ✅ | ✅ green |
...
| 131-02-T1/T2 then 131-04-T1/T2 | 131-02 then 131-04 | 1 then 2 | INST-03 | T-131-05, T-131-06, T-131-14, T-131-15, T-131-16 | One immutable operation plan validates all migration and managed destinations; fresh managed or migration collision leaves both artifact classes, directories, and manifest unchanged through the public install/upgrade entry points. | filesystem + public-orchestration integration | `mix test test/lockspire/install/migrations_test.exs test/integration/install_upgrade_test.exs test/lockspire/install/file_transaction_test.exs` | ✅ | ✅ green |
| 131-06-T1/T2 | 131-06 | 4 | INST-04 | T-131-22, T-131-23, T-131-24 | Every missing host seam is reported independently with actionable remediation; router shape and output stay fail-closed and redaction-safe. | unit + Mix task | `mix test test/lockspire/install/verify_test.exs test/mix/tasks/lockspire_verify_test.exs` | ✅ | ✅ green |
| 131-05-T1/T2 | 131-05 | 3 | INST-05 | T-131-18, T-131-19, T-131-20 | Default-profile proof succeeds without weakening defaults; FAPI proof remains explicit opt-in and persists through upgrade. | generated-host integration | `mix test test/integration/install_generator_test.exs test/integration/install_upgrade_test.exs` | ✅ | ✅ green |
| 131-01-T2 | 131-01 | 1 | INST-06 | T-131-03 | Rendered claims resolver compiles against the real `%Lockspire.Host.Claims{}` contract and honors configured logout behavior. | template compile + behavior | `mix test test/integration/install_generator_test.exs` | ✅ | ✅ green |
...
- [x] Compile a generated host router fixture that imports the rendered macro rather than spelling routes independently.
- [x] Render the consent template through public installation, assert byte parity with the committed executable fixture, compile the emitted module unchanged, confirm `Phoenix.Router.routes/1` selects it, and exercise its LiveView visit/submission against a stored interaction.
- [x] Add a bounded `with_test_source_root/2` package migration fixture seam that is process-scoped, test-environment-only, restored in `after`, absent from CLI switches, and used around the real public install/upgrade entry points.
- [x] Add immutable all-artifact operation-plan proof: fresh managed collision and fresh migration identity/content collision each preserve a complete pre/post tree snapshot with no new directories, managed files, migration files, or manifest.
- [x] Compile rendered resolver, consent LiveView, router macro, config, and default-smoke templates.
- [x] Separate default-profile generated smoke from explicitly selected FAPI proof.
...
- [x] Task feedback latency remains under 90 seconds.
- [x] Every high-severity threat has green named ASVS L1 automated evidence; unresolved high-severity threats block plan and phase completion.
- [x] `nyquist_compliant: true` set in frontmatter after implementation evidence exists.

**Approval:** validated by adversarial Nyquist audit on 2026-08-26.

## Dated Audit Evidence

| Date | Coverage | Command | Result |
|------|----------|---------|--------|
| 2026-08-26 | INST-01, INST-05, INST-06: rendered router/config/resolver/default/FAPI artifacts | `mix test test/integration/install_generator_test.exs` | green; actual installer output was compiled/executed, compiled route order inspected, configured logout behavior exercised, and FAPI remained opt-in. |
| 2026-08-26 | INST-03: migration preflight, public install/upgrade atomicity, journal recovery/rollback, and symlink refusal | `mix test test/integration/install_upgrade_test.exs test/lockspire/install/migrations_test.exs test/lockspire/install/file_transaction_test.exs` | green; fresh/repeat/upgrade/collision snapshots and hostile journal paths exercised real public orchestration. |
| 2026-08-26 | INST-04: independent config diagnostics, compiled route-shape verification, and safe Mix-task output | `mix test test/lockspire/install/verify_test.exs test/mix/tasks/lockspire_verify_test.exs` | green. |
| 2026-08-26 | INST-02: authoritative context, generated loading/error/terminal states, and approval-to-token flow | `mix test test/lockspire/web/consent_context_test.exs test/lockspire/web/live/consent_live_test.exs && mix test --include integration test/integration/phase6_onboarding_e2e_test.exs` | green; generated LiveView was reached by the compiled router and submitted through the supported controller endpoint. |
