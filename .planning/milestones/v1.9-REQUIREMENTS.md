...
- [x] **AUTHZ-01**: Add RSA/EC encryption keypairs (`enc`) to `Storage.KeyStore` and JWKS endpoints.
- [x] **AUTHZ-02**: Implement nested JWT validation (Sign-then-Encrypt) in `Protocol.Jar` using `JOSE.JWE` and `JOSE.JWS`.
...
|-------------|-------|--------|
| AUTHZ-01 | Phase 40 | Complete |
| AUTHZ-02 | Phase 40 | Complete |
