...
- **D-02:** `release_readiness_contract_test` adds one new clause that, for each of the four files, extracts the bytes between `BEGIN` and `END` markers, normalizes (left-strips the per-file uniform indent; strips the leading `# ` from each interior line on the Python file only), then `:crypto.hash(:sha256, normalized)` compares all four hashes — failing loudly with a diff if any pair differs.
  - **Clarification (revision iteration 1, WARNING #4):** by implication of D-10's commented-out-Elixir-inside-the-heredoc carrier, the same leading `# ` strip applies to the install template's `:elixir_in_commented_heredoc` carrier kind. So the strip is on TWO carrier kinds in practice: `:python_commented` (the Python smoke script) and `:elixir_in_commented_heredoc` (the install template heredoc). Both kinds carry leading `# ` on every interior line; both get the strip; the resulting post-normalization byte sequence is identical across all four files. The original "Python file only" wording was a slip — D-10 always required this extension to keep the four-file hash compare meaningful.
...
### Late-locked decisions (resolved from 97-RESEARCH.md Open Questions, 2026-05-27)

- **D-13:** The canonical pipeline block includes `audience: "billing-api"` as a placeholder value. This extends D-04's "pick a placeholder; reconcile in Phase 101" pattern to the `audience:` drift between the docs page (currently declares `audience: "billing-api"`) and the live demo router at `examples/adoption_demo/lib/adoption_demo_web/router.ex:23-27` (currently omits `audience:` entirely). Phase 101 DEMO-03 reconciles the demo router to consume the placeholder the same way DEMO-01/02/03 reconcile the `dpop_replay_store:` placeholder from D-04. Keeps the canonical block aligned with what DOCS-01's contract sentence implies (audience-bounded host APIs).
- **D-14:** The Python-comment carrier of the canonical block in `scripts/demo/adoption_smoke.py` lands **adjacent to the `exercise_authorization_code` flow at line 244**, not at the top of the file. Rationale: a reader looking at the protected-API smoke test sees the canonical block describing exactly what the smoke is protecting against. Markers `# BEGIN LOCKSPIRE_PROTECTED_PIPELINE` / `# END LOCKSPIRE_PROTECTED_PIPELINE` plus Python-comment-prefixed interior lines per D-03.
- **D-15:** The two secondary fenced Elixir blocks in `docs/protect-phoenix-api-routes.md` — currently the scope-restricted example (~L39-49) and the audience-restricted example (~L51-62) — get **rewritten to reference the canonical block** ("see the canonical pipeline above; this example narrows it to scopes:/audience: only") instead of restating the three plug names verbatim. Within-file restatements are still drift risk; D-12's letter doesn't reach them but its spirit does. The didactic value of the examples is preserved by the reference-to-canonical rewrite. Phase 97 lints itself: zero restatements of the three plug names outside the canonical block within `docs/protect-phoenix-api-routes.md`.
