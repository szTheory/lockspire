...
| 26-XX-XX | (audit attribution)       | TBD | DCR-22 | T-26-ATTRIBUTION | DCR codepaths emit `actor_type: "dcr"` or `"self_registered_client"`; regression test fails on `"operator"` for any DCR write | regression | `mix test test/lockspire/admin/dcr_audit_attribution_test.exs` | ❌ W0 | ⬜ pending |
...
- [ ] `test/lockspire/admin/dcr_audit_attribution_test.exs` — regression stub: scan emitted audit rows for `actor_type` and fail on `"operator"` for any DCR write (DCR-22)
