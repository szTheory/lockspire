...
| 139-11 — explicit lifecycle modes and authenticated planning proof | HYGIENE-06, TRUTH-03, TRUTH-05 | `node /Users/jon/.codex/gsd-core/bin/gsd-tools.cjs capability install ./tools/gsd-capabilities/lockspire-phase-finalizer --scope project --yes --raw`; portable: `LOCKSPIRE_GSD_HOST_FIXTURE=tools/gsd-capabilities/lockspire-phase-finalizer/fixtures/gsd-host-contract.json node --test tools/gsd-capabilities/lockspire-phase-finalizer/lockspire-finalize-command-router.test.cjs tools/gsd-capabilities/lockspire-phase-finalizer/lockspire-finalize-lifecycle.test.cjs`; installed: `GSD_TOOLS=/Users/jon/.codex/gsd-core/bin/gsd-tools.cjs node --test tools/gsd-capabilities/lockspire-phase-finalizer/lockspire-finalize-command-router.test.cjs tools/gsd-capabilities/lockspire-phase-finalizer/lockspire-finalize-lifecycle.test.cjs`; acceptance: `GSD_TOOLS=tools/gsd-capabilities/lockspire-phase-finalizer/fixtures/gsd-core/bin/gsd-tools.cjs ASDF_ELIXIR_VERSION=1.19.5-otp-28 ASDF_ERLANG_VERSION=28.1 MIX_ENV=test mix test test/lockspire/release/repository_hygiene_contract_test.exs --only phase139_final_acceptance --only phase139_acceptance_receipt` | PASS — both acceptance selectors 2 tests, 0 failures; portable and installed Node suites each 17 passed, 0 failed, 2 intentionally skipped; workflow lint and shell syntax pass. Planning consistency is stubbed only inside the authenticated Phase 140 `plan:pre` fixture; live synchronized-main receipt and Phase 140 CI-06/CI-07 entry evidence remain deferred |
...
| G-139-06 | Truth 13 — execute:post publishes one current ledger before verification | Plan 139-11 portable fixture mode and explicit installed-runtime parity mode; active files are regenerated by the supported capability installer before exact byte/render checks; both suites passed 17 tests with 2 expected integration skips | AUTOMATED_LIVE |
| G-139-07 | Truth 14 — supported post-transition gate authenticates sealed lifecycle before Phase 140 | Plan 139-11 authenticated Phase 140 `plan:pre` fake-remote fixture proves the planning consistency command runs only after receipt/relation validation and before refs can move; production hook identity remains unchanged | AUTOMATED |
...
Current-host parity remains an execution boundary: live rendering checks compare the active capability with the same tracked fixture after the supported installer refreshes from tracked source. `LOCKSPIRE_GSD_HOST_FIXTURE` alone selects portable contract mode even when a home-directory GSD runtime is discoverable; an explicit `GSD_TOOLS` selects installed parity mode. The portable CI fixture is test input only and does not install GSD or authorize runtime behavior. Plan 139-11's planning-consistency test runs in the existing blocking Phase 140 `plan:pre` hook only after canonical verification, transition receipt, and sealed relation authentication; a failure leaves pending receipt recovery intact and occurs before ref movement. G-139-01 and G-139-09 remain `PENDING_EXTERNAL` until the supported post-transition path authenticates the synchronized SHA and retains a valid mode-0600 receipt outside the worktree.

**Plan 139-11 current evidence (2026-09-26):** The supported capability installer reports project capability `lockspire-phase-finalizer` active at version `1.2.0`; installed runtime bytes match tracked files. The portable fixture command and explicit installed-runtime command each report 17 passing tests, 0 failures, and 2 existing integration skips. Exact acceptance selectors report 2 tests, 0 failures; their fake `mix` asserts the exact pinned command and environment, runs only after receipt/relation authentication, and proves command failure preserves refs and pending receipt. These fixtures do not run the state-dependent planning test itself or create live synchronized-main receipt evidence.
