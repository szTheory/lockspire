...
---
*Last updated: 2026-05-28 — Phase 99 (signer extraction + JWT-default issuance) complete: `Lockspire.Protocol.AccessTokenSigner` now owns the single RFC 9068 `at+jwt` signing site for all five grant paths (AC, refresh, device, CIBA, RFC 8693); the default access-token format flipped to `:jwt` with a runtime-settable server default and nullable per-client override (admin-editable), coherent `aud` semantics (list for grant paths, bare-string for token-exchange), and discovery now advertises `access_token_signing_alg_values_supported` (SIGNER-01/02, FORMAT-01/02, AUD-01/02/03, DISCOVERY-01). Prior footer — Phase 98 (plug-hardening): `Lockspire.Plug.VerifyToken` rejects opaque tokens at the front edge, enforces RFC 9068/8725 compliance with distinct reason codes, derives the WWW-Authenticate challenge from token binding, and structurally closes cross-API audience reuse (VERIFIER-01..06).*
