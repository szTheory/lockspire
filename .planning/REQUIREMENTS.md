...
- [x] **CONF-02**: A scheduled repo-native conformance lane retains redacted evidence and remains supplemental until measured reliability justifies any stronger gate or certification claim.
- [x] **REL-01**: Release automation proves a clean-room install, migration, verification, boot, and minimal HTTP journey against the built artifact before publish and the exact public version after publish.
- [x] **REL-02**: Maintainers can verify that the published Hex checksum matches the produced artifact and can inspect a retained redacted release manifest with pinned runtime/tooling versions.
